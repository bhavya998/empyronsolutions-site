...
const steps = [
    {
        icon: <MessageSquare className="w-7 h-7 text-brand" />,
        step: '01',
    ...
    {
        icon: <Search className="w-7 h-7 text-brand-deep" />,
        step: '02',
    ...
    {
        icon: <Cpu className="w-7 h-7 text-brand" />,
        step: '03',
        title: 'Build & Iterate',
    ...
    {
        icon: <Rocket className="w-7 h-7 text-brand-deep" />,
        step: '04',
        title: 'Ship & Support',
...
export const HowIWork: React.FC = () => {
    return (
        <section id="process" className="py-24 relative z-10 bg-brand-dark overflow-hidden border-t border-white/5">
            {/* Soft ambient glow */}
            <div className="absolute top-1/3 -right-40 w-[500px] h-[400px] bg-brand-deep/5 blur-[130px] rounded-full pointer-events-none" />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-6xl mx-auto">
                    {steps.map((step, i) => (
                        ...
                        >
                            ...
                            <span className="text-xs font-medium text-brand-light bg-brand/10 px-3 py-1 rounded-full">{step.badge}</span>
                        </motion.div>
                    ))}
                </div>
            </div>
